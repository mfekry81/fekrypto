...
function connectWebSocket() {
    ...
    const streams = allSymbols.slice(0, 50).map(symbol => // Limit to first 50 symbols to avoid URL length issues
...
async function fetchHistoricalDataForAllSymbols() {
    ...
    for (let i = 0; i < Math.min(allSymbols.length, 50); i += batchSize) { // Limit to 50 symbols
    ...
    for (const batch of batches) {
        ...
        updateConnectionStatus('connecting', 
            `Loading data... ${processedCount}/${Math.min(allSymbols.length, 50)}`);
    ...
    // Calculate initial technical indicators for all symbols
    allSymbols.slice(0, 50).forEach(symbol => {
        calculateTechnicalIndicators(symbol);
    });
